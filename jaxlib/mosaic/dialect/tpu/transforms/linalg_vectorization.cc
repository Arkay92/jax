...
struct ContractionBitwidthConvert
  ...
  LogicalResult matchAndRewrite(vector::ContractionOp op,
    ...
    if (!op.getLhsType().getElementType().isBF16()) {
      return rewriter.notifyMatchFailure(op, "argument is not bf16");
    }
    ShapedType acc_ty = dyn_cast<ShapedType>(op.getAccType());
    if (!acc_ty.getElementType().isBF16()) {
      return rewriter.notifyMatchFailure(op, "accumulator is not bf16");
    }
    Value lhs = op.getLhs();
    Value rhs = op.getRhs();
    if (!supports_bf16_matmul_) {
      // In the case current target does not support bf16 matmul, we need to
      // extend the lhs and rhs to f32.
      lhs = rewriter.create<arith::ExtFOp>(
          op.getLoc(),
          VectorType::get(acc_ty.getShape(), rewriter.getF32Type()), lhs);
      rhs = rewriter.create<arith::ExtFOp>(
          op.getLoc(),
          VectorType::get(acc_ty.getShape(), rewriter.getF32Type()), rhs);
    }
    vector::ContractionOp contraction = rewriter.create<vector::ContractionOp>(
        ...
        rewriter.create<arith::ExtFOp>(
            ...
            op.getAcc()),
        op.getIndexingMaps(), op.getIteratorTypes(), op.getKind());
